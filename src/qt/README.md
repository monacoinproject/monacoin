This directory contains the source code for the Monacoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- ClientModel is used by the main application `monacoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a monacoin:... link from a browser).
...
* UI elements like MonacoinAmountField, which inherit from QWidget.
* `monacoinstrings.cpp`: automatically generated
* `monacoinunits.(h/cpp)`: MONA / mMONA / etc. handling
* `callback.h`
...
**Note:** Do not change `local/monacoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `monacoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `monacoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `monacoin-qt`
9. You're all set! Start developing, building, and debugging the Monacoin Core GUI
