...
EXAMPLE_CONF_FILE=${EXAMPLE_CONF_FILE:-$SHARE_EXAMPLES_DIR/monacoin.conf}

[ ! -x "$MONACOIND" ] && echo "$MONACOIND not found or not executable." && exit 1

DIRTY=""
VERSION_OUTPUT=$($MONACOIND --version)
if [[ $VERSION_OUTPUT == *"dirty"* ]]; then
  DIRTY="${DIRTY}${MONACOIND}\n"
fi

if [ -n "$DIRTY" ]
then
  echo -e "WARNING: $MONACOIND was built from a dirty tree.\n"
  echo -e "To safely generate a monacoin.conf file, please commit your changes to $MONACOIND, rebuild, then run this script again.\n"
fi

echo 'Generating example monacoin.conf file in share/examples/'
...
## Generated by contrib/devtools/gen-monacoin-conf.sh.
##
...
# parse the output from monacoind --help
