...
class TestNode():
    ...
    def __init__(self, i, dirname, extra_args, rpchost, timewait, binary, stderr, mocktime, coverage_dir):
        ...
        if timewait:
            ...
            self.rpc_timeout = 60
        if binary is None:
            self.binary = os.getenv("MONACOIND", "monacoind")
        else:
            self.binary = binary
        self.stderr = stderr
        ...
        self.cli = TestNodeCLI(os.getenv("MONACOINCLI", "monacoin-cli"), self.datadir)
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            assert self.process.poll() is None, "monacoind exited with status %i during initialization" % self.process.returncode
            try:
        ...
        raise AssertionError("Unable to connect to monacoind")
    ...
    def node_encrypt_wallet(self, passphrase):
        ...
        This causes monacoind to shutdown, so this method takes
...
class TestNodeCLI():
    ...
    def __call__(self, *args, input=None):
        # TestNodeCLI is callable with monacoin-cli command-line args
