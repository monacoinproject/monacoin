# CJDNS support in Monacoin Core

It is possible to run Monacoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Monacoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Monacoin network and individual nodes.
...
before it will work with your Monacoin Core node. This is described in steps
...
## Run Monacoin Core with CJDNS

Once you are connected to the CJDNS network, the following Monacoin Core
...
When enabled, this option tells Monacoin Core that it is running in an
...
IPv6 local network. This helps Monacoin Core perform better address management:
...
CJDNS support was added to Monacoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `monacoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/monacoin/monacoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Monacoin Core

There are several ways to see your CJDNS address in Monacoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `monacoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `monacoin-cli getpeerinfo`).
