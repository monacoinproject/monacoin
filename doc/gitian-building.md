...
*Setup instructions for a Gitian build of Monacoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Monacoin
...
- [Building Monacoin Core](#building-monacoin-core)
...
Clone the git repositories for monacoin and Gitian.
...
in the monacoin repository under 'Fetch and create inputs' to install sources which require
...
To build Monacoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the monacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/monacoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/monacoin-project/monacoin
...
For example:
```bash
URL=https://github.com/thrasher-/monacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit monacoin=${COMMIT} --url monacoin=${URL} ../monacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit monacoin=${COMMIT} --url monacoin=${URL} ../monacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit monacoin=${COMMIT} --url monacoin=${URL} ../monacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the monacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../monacoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/monacoin
SIGPATH=/some/root/path/monacoin-detached-sigs

./bin/gbuild --url monacoin=${BTCPATH},signature=${SIGPATH} ../monacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
in `gitian.sigs.mona` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/monacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/monacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/monacoin-osx-build.assert
```
...
[gitian.sigs.mona](https://github.com/monacoin-project/gitian.sigs.mona/) repository, or if that's not possible create a pull
