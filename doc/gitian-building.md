...
*Setup instructions for a gitian build of Monacoin using a Debian VM or physical system.*
...
- [Building Monacoin](#building-monacoin)
...
Clone the git repositories for monacoin and gitian and then checkout the monacoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the monacoin repository
...
To build Monacoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the monacoin repository.
...
For example:
```bash
URL=https://github.com/user/monacoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/monacoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/monacoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/monacoin-build.assert
```
...
[monacoin/gitian.sigs](https://github.com/monacoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
