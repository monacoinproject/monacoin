...
#### Removal of BIP61 reject network messages from Monacoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Monacoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Monacoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Monacoin Core. Monacoin Core logs debug messages
...
- OpenSSL is no longer used by Monacoin Core.  (#17265)

- BIP70 support has been fully removed from Monacoin Core. The
...
- All custom settings configured when Monacoin Core starts are now
...
- The "Start Monacoin Core on system login" option has been removed on macOS.
  (#17567)
...
- The `monacoin-cli` utility used with the `-getinfo` parameter now
...
- The `monacoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `monacoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_monacoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBMONACOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_monacoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBMONACOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for monacoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate monacoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest monacoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_monacoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (monacoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone monacoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Monacoin Core files (hebasto)
...
- #17119 Fix broken monacoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from monacoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.monacoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to monacoin-wallet (hel-o)
...
- #18341 Replace remaining literal MONA with `CURRENCY_UNIT` (domob1812)
...
- #16115 On monacoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from monacoin-cli -getinfo (malevolent)
