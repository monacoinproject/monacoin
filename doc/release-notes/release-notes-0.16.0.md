Monacoin Core version 0.16.0 is now available from:

  <https://monacoincore.org/bin/monacoin-core-0.16.0/>
...
GUI changes
-----------
- Uses of "µMONA" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest monacoin.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.monacoin.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for monacoin-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to monacoin-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µMONA to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_monacoin_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` monacoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.monacoin doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for monacoin-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in monacoin-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop monacoind (OmeGak)
...
- #10529 `331352f` Improve monacoind systemd service file (Flowdalic)
