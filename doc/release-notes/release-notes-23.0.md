...
Monacoin Core version 23.0 is now available from:

  <https://monacoincore.org/bin/monacoin-core-23.0/>
...
- A monacoind node will no longer rumour addresses to inbound peers by default.
  They will become eligible for address gossip after sending an ADDR, ADDRV2,
  or GETADDR message. (#21528)

- Before this release, Monacoin Core had a strong preference to try to connect only to peers that listen on port 8333. As a result of that, Monacoin nodes listening on non-standard ports would likely not get any Monacoin Core peers connecting to them. This preference has been removed. (#23542)

- Full support has been added for the CJDNS network. See the new option `-cjdnsreachable` and [doc/cjdns.md](https://github.com/monacoin/monacoin/tree/23.x/doc/cjdns.md) (#23077)
...
Monacoin Core release binaries for Linux now include experimental tracepoints which
...
[doc/tracing.md](https://github.com/monacoin/monacoin/blob/23.x/doc/tracing.md) and
usage examples are provided in [contrib/tracing/](https://github.com/monacoin/monacoin/tree/23.x/contrib/tracing).
...
- The top-level fee fields `fee`, `modifiedfee`, `ancestorfees` and `descendantfees`
  ...
  fields in the `fees` object are denominated in MONA. (#22689)
...
* On startup, the list of banned hosts and networks (via `setban` RPC) in
  `banlist.dat` is ignored and only `banlist.json` is considered. Monacoin Core
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from Monacoin Core in 22.0. (#22544)
