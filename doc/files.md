...
`./`               | `monacoin.conf`        | Contains [configuration settings](monacoin-conf.md) for `monacoind` or `monacoin-qt`; can be specified by `-conf` option
...
These subdirectories and files are no longer used by the Monacoin Core:
